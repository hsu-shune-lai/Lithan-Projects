...
<h2>Edit form for Enclosures</h2>
<form method="POST" action="Enclosures_update.jsp">
<%
	String sId = request.getParameter("Enclosures_ID");
		if (sId == null){
			out.println("Enclosures ID is not found");
		}else {
			int id = Integer.parseInt(sId);
			EnclosuresManage dao = new EnclosuresManage();
			try {
				Enclosures c = dao.getById(id);
				//out.print(c);
%>
			<input type ="hidden" name="enclosures_id" value="<%=c.getEnclosures_ID() %>"> <br><br>
			
			<label for="enclosures_type">Enclosures Type</label><br>
			<input type="text" name="enclosures_type" id="enclosures_type" value="<%=c.getEnclosures_Type() %>" required><br><br>
			
			<label for="location">Location</label><br>
			<input type="text" name="location" id="location" value="<%=c.getLocation() %>" required><br><br>
